 ...
 * 
 * Este programa implementa o método de Jacobi para resolver sistemas
...
/**
 * Função para resolver um sistema linear pelo método de Jacobi
 ...
 */
bool metodo_jacobi(double A[20][20], double b[20], double x[20], int n, double e, int max_iter) {
    double x_novo[20];
    double soma;
    int i, j, iter;
    ...
    for (iter = 1; iter <= max_iter; iter++) {
        ...
        for (i = 0; i < n; i++) {
            std::cout << "x" << i + 1 << " = " << std::fixed << std::setprecision(12) << x_novo[i] << std::endl;
        }
        
        // Verificando convergência
        bool convergiu = true;
        for (i = 0; i < n; i++) {
            if (fabs(x_novo[i] - x[i]) > e) {
                convergiu = false;
                break;
            }
        }
...
int main() {
    ...
    std::cout << "MÉTODO DE JACOBI PARA RESOLUÇÃO DE SISTEMAS LINEARES" << std::endl;
    std::cout << "===============================================" << std::endl;
    ...
    bool sucesso = metodo_jacobi(A, b, x, n, e, max_iter);
